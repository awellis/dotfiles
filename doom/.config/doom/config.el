...
;; Merge (don't `setq' the whole list): a wholesale replace wipes Doom's own
;; default-frame-alist entries, notably (vertical-scroll-bars) which disables
;; the scroll bars, plus menu-bar-lines/tool-bar-lines.
(dolist (pair '((background-color . "#1e1e2e")
...
;; Doom's :ui ligatures (+extras +fira) already enables global-ligature-mode
;; with the standard programming ligatures. ligature.el unions the `t' set
;; with each mode's set, so we only add the R/Julia operators here.
(after! ligature
...
(use-package! claude-code-ide
  ...
  ;; Expose Emacs' own intelligence to Claude over MCP: xref, imenu,
  ;; tree-sitter and project info. Without this the MCP server runs but
  ;; Claude gets no Emacs-side tools.
...
;; diff-hl: skip TRAMP/remote files. Doom's :ui vc-gutter already limits
;; diff-hl to version-controlled buffers, so no custom guard is needed.
(after! diff-hl
...
;; Unified AI assistant keybindings under SPC k.
;; Claude Code and pi get parallel bindings, so the same key means the same
;; thing whichever agent you are driving: r = send region, b = switch to its
;; buffer, R = resume, k = stop.
(map! :leader
      (:prefix ("k" . "AI")

       (:prefix ("c" . "claude code")
        :desc "Menu (transient)"       "c" #'claude-code-ide-menu
        :desc "Start in project"       "o" #'claude-code-ide
       ...
       (:prefix ("p" . "pi")
        :desc "Start pi agent"         "p" #'agent-shell-pi-start-agent
        :desc "Send region"            "r" #'agent-shell-send-region
        :desc "Send dwim"              "s" #'agent-shell-send-dwim
        :desc "Send current file"      "f" #'agent-shell-send-current-file
        :desc "Switch to shell"        "b" #'agent-shell-switch-buffer
...
;; Global entry point to the Claude transient, independent of the leader.
(map! :n "C-c c m" #'claude-code-ide-menu
      :i "C-c c m" #'claude-code-ide-menu
      :v "C-c c m" #'claude-code-ide-menu)
...
(use-package! copilot
  ...
  ;; Silence copilot's indentation warning; its defaults are fine.
  (setq copilot-indent-offset-warning-disable t))

;; Copilot sits under the same SPC k AI prefix, but as its own subgroup:
;; a flat "p" here would shadow the whole SPC k p (pi) keymap.
(map! :leader
      (:prefix ("k" . "AI")
       (:prefix ("o" . "copilot")
        :desc "Toggle Copilot"   "t" #'copilot-mode
        :desc "Copilot diagnose" "d" #'copilot-diagnose
...
;;; Global Flyspell toggle (all buffers at once; text=full words, code=comments/strings).
;;; Doom also provides a per-buffer toggle on `SPC t s'.
(defvar my/spell-global-enabled nil)
(defun my/apply-spell-to-buffer ()
  (cond ((derived-mode-p 'text-mode)
         (flyspell-mode (if my/spell-global-enabled 1 -1)))
        ((and my/spell-global-enabled (derived-mode-p 'prog-mode))
         (flyspell-prog-mode))
        ((bound-and-true-p flyspell-mode) (flyspell-mode -1))))
(defun my/toggle-spell-checking ()
  "Toggle Flyspell across all current and future buffers."
  (interactive)
